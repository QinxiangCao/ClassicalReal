...
Theorem D2C_property10:forall (x:D1.Real),D2C (Dedekind_Rabs x)==Rabs (D2C x).
Proof.
...
(n>=N)%nat -> forall A, 
(match Rseq with DRseq_intro RS _ => RS end) n A -> (Dedekind_Rabs (D3.Rminus A  Lim)< Eps)%D.
(* Definition of D.Rlimit *)

Definition D2C_seq_pre (RS:nat->D1.Real->Prop):nat->C1.Real->Prop:=
fun n x=>exists (y:D1.Real),RS n y/\D2C y==x.
