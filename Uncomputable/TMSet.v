...
Module TM_0_0.
  ...
  Axiom Turing_proper1 : forall (i j k: nat), (j <= k)%nat -> TM i j = true -> TM i k = true.
  Theorem Turing_proper2 : forall (i j k: nat), (k <= j)%nat -> TM i j = false -> TM i k = false.
  Proof.
    unfold not in *.
    intros.
...
Module TM_1_2.
  ...
  Axiom TM_pro0 : forall (f : nat -> (nat * nat)) , exists (i : TM12), forall (x : nat) , exists (j : nat),
                  eval_1_2 i x j = Some (f x).
  Axiom TM_pro1 : forall (i : TM12) (x j1 j2: nat)(z : nat * nat), (j1 >= j2)%nat -> eval_1_2 i x j2 = Some z -> eval_1_2 i x j1 = Some z.
  Theorem TM_pro1' : forall (i : TM12) (x j1 j2 : nat) , (j1 <= j2)%nat -> eval_1_2 i x j2 = None -> eval_1_2 i x j1 = None.
  Proof.
    intros.
    destruct (eval_1_2 i x j1) eqn:H1 ; auto.
    pose proof TM_pro1 i x j2 j1 p H H1.
    rewrite H0 in H2.
    discriminate H2.
  Qed.
