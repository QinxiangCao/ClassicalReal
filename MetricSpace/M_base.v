...
Proof.
  intros.  apply HDD_pre. apply mle.
Qed.
...
Theorem ms_trans : Metric equCX equCA leCX plusCX zeroCX pfCX
        (@distC _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ HDD_pre HPD _).
Proof.
    split.
    -intros. destruct(classic (equCA p1 p2)).
     ...
     -intros. destruct p1,p2,p3,p4. simpl. intro. simpl in H0,H. intros.
      ...
      apply mtr.
      assert (distX (dist a b) (dist a b0) <= dist b b0).
      specialize(HDD_l X eqX leX plusX zeroX _ A eqA _
                       dist distX _ _ HPD _ HDD_pre).
      intros. apply H11.
      assert (distX (dist a b0) (dist a0 b0) <= dist a a0).
      specialize(HDD_l X eqX leX plusX zeroX _ A eqA _
                       dist distX _ _ HPD _ HDD_pre).
      intros.
      repeat (rewrite (msy _ b0)).
      apply H12.
      assert(dist b b0 + dist a a0 < eps). rewrite <-Heq. 
      pose proof (lt_two_plus_two X eqX _ leX plusX zeroX).
      destruct H13 with (a := dist b b0) (b := dist a a0) (c := eps2) (d := eps1).
      ...
      assert (distX (dist a b) (dist a0 b0) <= dist b b0 + dist a a0).
      apply poft with
          (b1 := distX (dist a b) (dist a b0) +
                 distX (dist a b0) (dist a0 b0)).
      auto.
      apply (le_two_plus_two X eqX _ leX plusX _);auto.
      destruct (le_lt_eq _ _ _ (distX (dist a b) (dist a0 b0)) (dist b b0 + dist a a0)).
      apply H15 in H14. destruct H14. apply lttr with (y := dist b b0 + dist a a0);auto.
      rewrite (msy a0 b0). apply lt_intro;destruct H13;auto.
      auto. rewrite (msy b0 a0); auto.
      rewrite (msy b0 a0);apply H14.
      apply lt_intro;apply H13.
      rewrite  H14.
      apply lt_intro; destruct H13; auto.
Qed.
