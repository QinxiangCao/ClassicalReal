...
Proof. unfold reflexive. unfold Real_equiv.
  intros. destruct x. unfold Cauchy in *.
  intros eps. exists 0%nat. intros. unfold Qminus.
  rewrite Qplus_opp_r. apply H0.
Qed.

Theorem Real_def_symm: symmetric Real Real_equiv.
Proof. unfold symmetric. unfold Real_equiv.
  intros. destruct x as [Seqa Ha]. destruct y as [Seqb Hb].
...
Proof. unfold transitive. unfold Real_equiv.
  ...
  assert (H7: eps == eps *(1#2) + eps *(1#2)) by ring.
...
Proof. intros. unfold Cauchy. intros. exists 1%nat. intros m1 m2 [H1 H2].
  unfold Qminus. rewrite Qplus_opp_r. simpl. apply H.
Qed.
...
assert (Hn: le n1 n2 \/ ~ (le n1 n2)). { apply classic. } destruct Hn.
  * exists n2. intros. assert (H7: (m1 > n1 /\ m2 > n1)%nat). { omega. } 
    ...
    assert (H': A m1 + B m1 - (A m2 + B m2) == (A m1 - A m2) + (B m1 - B m2)) by ring.
...
Proof. intros A1 A2 B1 B2 Heqa Heqb. unfold Real_equiv.
  ...
  assert (H6: eps == eps *(1#2) + eps *(1#2)) by ring.
...
assert (Hn: le na nb \/ ~ (le na nb)). { apply classic. } destruct Hn.
  * exists nb. intros. assert (H5: (m > na)%nat). { omega. }
  ...
  assert (H': x m + x1 m  -(x0 m + x2 m) == (x m - x0 m) + (x1 m - x2 m)) by ring.
  rewrite H'.
...
Proof. intros. destruct A as [A Ha] ,B as [B Hb]. unfold Rplus.
  ...
  exists O. intros. unfold Qminus. rewrite Qopp_plus. rewrite Qplus_assoc.
...
Proof. intros. destruct A as [A Ha] ,B as [B Hb], C as [C Hc]. unfold Rplus.
  ...
  unfold CauchySeqPlus. clear Ha Hb.
  exists O. intros. rewrite Qplus_assoc. unfold Qminus.
...
Proof. intros. destruct A as [A Ha]. unfold Rzero. unfold Rplus.
  ...
  exists O. intros. rewrite Qplus_0_r. unfold Qminus. rewrite Qplus_opp_r. apply H.
Qed.
...
Proof. intros. unfold Cauchy in *. unfold Cauchy_opp. intros.
  ...
  exists n. intros. assert (H':- A m1 - - A m2 == A m2 - A m1) by ring.
  rewrite H'.
  rewrite Qabs_Qminus. apply Hn. apply H0.
Qed.
